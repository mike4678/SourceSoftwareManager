...
//保留服务器性能测试结果
foreach ($_POST as $post=>$value) //把参数传入变量key中
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		acd2 = new Date();
		acd2ok=acd2.getTime();
		window.location = '?/system/info/?speed=' +(acd2ok-acd1ok)+'#w_networkspeed';
	</script>
<?php
}
//网速测试-结束
...
elseif($_GET['/system/info/?speed']=="0")
{
	$speed=6666.67;
}
elseif(isset($_GET['/system/info/?speed']) and $_GET['/system/info/?speed']>0)
{
	$speed=round(100/($_GET['/system/info/?speed']/1000),2); //下载速度：$speed kb/s
}
else
{
...
<style type="text/css">
<!--
.page{text-align: center;font-size:12px;font-family: "Microsoft Yahei",Tahoma, Arial}
.w_logo{height:25px;text-align:center;color:#333;FONT-SIZE: 15px;  font-size:12px;font-family: "Microsoft Yahei",Tahoma, Arial;}
.w_top{height:25px;text-align:center; width:8.7%; font-size:12px;font-family: "Microsoft Yahei",Tahoma, Arial;}
...
tr{padding: 0; background:#FFFFFF;font-size:12px;font-family: "Microsoft Yahei",Tahoma, Arial;}
td{padding: 3px 6px; border:1px solid #CCCCCC;font-size:12px;font-family: "Microsoft Yahei",Tahoma, Arial;}
...
<!--<table><tr style='text-align:center;' ><th ><?php //echo $title . " " .$version ;?></th></tr></table>-->
	...
	<table>
		<tr id="page">
			...
			<th width="11%" class="w_top"><a href="#w_module">组件支持</a></th>
			<th width="11%" class="w_top"><a href="#w_module_other">第三方组件</a></th>
			<th width="11%" class="w_top"><a href="#w_db">数据库支持</a></th>
			<th width="11%" class="w_top"><a href="#w_networkspeed">网速检测</a></th>
			<th width="11%" class="w_top"><a href="#w_MySQL">MySQL检测</a></th>
			<th width="11%" class="w_top"><a href="#w_function">函数检测</a></th>
			<th width="11%" class="w_top"><a href="#w_mail">邮件检测</a></th>
		</tr>
	</table>
...
<table>
  ...
  <tr>
    ...
    <td width="27%"><?php $os = explode(" ", php_uname()); echo $os[0];?> &nbsp;内核版本：<?php if('/'==DIRECTORY_SEPARATOR){echo $os[2];}else{echo $os[1];} ?></td>

    <td width="12%">服务器解译引擎</td>

    <td width="46%"><?php echo $_SERVER['SERVER_SOFTWARE'];?></td>
...
<table>
  ...
  <tr>
    ...
    <td width="15%" >服务器当前时间</td>

    <td width="18%" ><span id="stime"><?php echo $stime;?></span></td>

    <td width="40%" >服务器已运行时间</td>

    <td width="27%" colspan="3"><span id="uptime"><?php echo $uptime;?></span></td>

  </tr>
  <tr>

    <td width="15%">CPU型号 [<?php echo $sysInfo['cpu']['num'];?>核]</td>
...
<table>
  ...
  <tr>
    ...
    <td width="31%">FTP支持：</td>

    <td width="19%"><?php echo isfun("ftp_login");?></td>

    <td width="31%">XML解析支持：</td>

    <td width="19%"><?php echo isfun("xml_set_object");?></td>
...
<table>
  ...
  <tr>
    <td width="31%">Zend版本</td>
    <td width="19%"><?php $zend_version = zend_version();if(empty($zend_version)){echo '<font color=red>×</font>';}else{echo $zend_version;}?></td>
    <td width="31%">
<?php
...
<table width="26%">
  ...
  <tr>
    ...
    <td width="31%">MySQL 数据库：</td>

    <td width="19%"><?php echo isfun("mysql_close");?>
    ...
    if(function_exists("mysql_get_server_info")) {
	    ...
	    $c = '&nbsp; 客户端 版本：'.@mysql_get_client_info();
    ...
    <td width="31%">ODBC 数据库：</td>

    <td width="19%"><?php echo isfun("odbc_close");?></td>
...
<form action="<?php echo "http://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']."#bottom";?>" method="post">
...
<table>
  ...
  <?php echo (isset($_GET['/system/info/?speed']))?"下载1000KB数据用时 <font color='#cc0000'>".$_GET['/system/info/?speed']."</font> 毫秒，下载速度："."<font color='#cc0000'>".$speed."</font>"." kb/s，需测试多次取平均值，超过10M直接看下载速度":"<font color='#cc0000'>&nbsp;未探测&nbsp;</font>" ?>
...
<table>
  ...
  ?>
	...
	<?php $run_time = sprintf('%0.4f', microtime_float() - $time_start);?>
		    处理请求时间: <?php echo $run_time?> 秒  内存使用量： <?php echo memory_usage();?> </p>
<a name="gobottom"></a>
<div class="go">
