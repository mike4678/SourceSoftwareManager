...
客户端，服务端及管理端部署方式与易语言版几乎相同，同样支持一键部署
...
1.Web管理端目前仅支持Windows 模式下搭建

2.部署很简单，在IIS中添加整个目录，如果使用apache或者一键端工具，则只需要像正常网站一样，设置好文件目录即可
...
为了便于切换，在火山版的目录下有一个“数据库更新”的文件夹，如果需要从1.0迁移至2.0，只需要先将程序配置好，然后将该目录下的sql文件导入，即可完成升级
...
如果检测到更新，则客户端下载更新包并缓存再在update目录下，待下一次运行时自动执行更新操作

详细更新修改方法

1.客户端代码变更后，在“全局变量类”中，修改Version及BuildData信息，然后将NVersion 修改为当前数值更高一个值，比如原本是4，可以更改成5，6...，该数字不能高于服务端上的数字
