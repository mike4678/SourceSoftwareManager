...
public final class CustomActivityOnCrash {
    ...
    private static final Deque<String> activityLog = new ArrayDeque<String>(MAX_ACTIVITIES_IN_LOG);
    ...
    public static String getAllErrorDetailsFromIntent(@NonNull Context context, @NonNull Intent intent) {
        ...
        errorDetails += "程序版本: " + versionName + " \n";
        if (buildDateAsString != null) {
            errorDetails += "程序构建日期: " + buildDateAsString + " \n";
        }
        errorDetails += "当前日期: " + dateFormat.format(currentDate) + " \n";
        ...
        errorDetails += "设备: " + getDeviceModelName() + " \n \n";
        errorDetails += "堆栈跟踪(崩溃信息):  \n";
    ...
    private static Class<? extends Activity> getErrorActivityClassWithIntentFilter(@NonNull Context context) {
        ...
        if (resolveInfos != null && resolveInfos.size() > 0) {
            ResolveInfo resolveInfo = resolveInfos.get(0);
            try {
				Log.e("Java", resolveInfo.activityInfo.name);
                return (Class<? extends Activity>) Class.forName(resolveInfo.activityInfo.name);
            } catch (ClassNotFoundException e) {
                //Should not happen, print it to the log!
